...
import type { Trainer, Badge } from './TrainersView'
...
export default function TrainerForm({ onSave, onUpdate, onDelete, existingTrainers }: Props) {
  ...
  const [formData, setFormData] = useState<Omit<Trainer, 'id'>>({
    ...
    description: '',
    badges: JOHTO_BADGES_DATA.map(b => ({ ...b, obtained: false })),
  })
  ...
  const handleEdit = (trainer: Trainer) => {
    setFormData({
      name: trainer.name,
      favoritePokemon: trainer.favoritePokemon,
      favoritePokemonImage: trainer.favoritePokemonImage || '',
  ...
  const handleDelete = (id: number) => {
    if (window.confirm('¿Eliminar este entrenador?')) {
      onDelete(id)
      resetForm()
      showNotification('Entrenador eliminado correctamente')
    }
  }

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.name.trim() || !formData.favoritePokemon) {
      showNotification('Nombre y Pokémon favorito son requeridos', 'error')
      return
    }
    
    if (isEditing && editingId !== null) {
      onUpdate({ ...formData, id: editingId })
      showNotification('Entrenador actualizado correctamente')
    } else {
      onSave(formData)
      showNotification('Entrenador creado correctamente')
    }
    resetForm()
  }
  ...
  const toggleBadge = (index: number) => {
    setFormData(prev => ({
      ...
      badges: prev.badges.map((badge, i) =>
        i === index ? { ...badge, obtained: !badge.obtained } : badge
      ),
    }))
  }
  ...
  const selectPokemon = (pokemon: PokemonEntry) => {
    setFormData(prev => ({
      ...
      favoritePokemonImage: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemon.id}.png`,
    }))
    setShowPokemonPicker(false)
  }
  ...
  const obtainedBadges = formData.badges.filter(b => b.obtained).length
    ...
    <div className="animate-fade-up">
      ...
      <div className="mb-8">
        <p className="text-xs tracking-widest uppercase mb-1 font-semibold" style={{ color: '#4fc3f7', letterSpacing: '0.2em' }}>
          {isEditing ? 'Modificar entrenador' : 'Nuevo entrenador'}
        </p>
        <h2 className="text-2xl md:text-3xl font-bold" style={{ color: '#e8eaf6' }}>
          {isEditing ? 'Editar perfil de entrenador' : 'Crear nuevo entrenador'}
        </h2>
      </div>

      {/* Form */}
      <form onSubmit={handleSubmit} id="trainer-form" className="space-y-6">
        {/* Basic Info Card */}
        <div className="glass-card p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  style={{
                    ...
                    color: formData.favoritePokemon ? '#e8eaf6' : '#64748b',
                  }}
                >
                  {formData.favoritePokemonImage && (
                    <img
                      src={formData.favoritePokemonImage}
                      alt={formData.favoritePokemon}
                  ...
                  <span className="flex-1 font-semibold">
                    {formData.favoritePokemon || 'Seleccionar Pokémon...'}
                  </span>
              ...
              {/* Pokemon Picker Dropdown */}
              {showPokemonPicker && (
                <div className="mt-2 glass-card p-4 max-h-80 overflow-y-auto">
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={e => setSearchQuery(e.target.value)}
                    placeholder="Buscar Pokémon..."
                    className="dark-input w-full px-3 py-2 rounded-lg text-sm mb-3"
                    autoFocus
                  />
                  <div className="grid grid-cols-3 gap-2">
                    {filteredPokemon.map(pokemon => (
                      <button
                        ...
                        className="flex flex-col items-center p-2 rounded-lg transition-all hover:bg-white/5"
                        style={{
                          border: formData.favoritePokemon === pokemon.name
                      ...
                      >
                        <img
                          ...
                          crossOrigin="anonymous"
                        />
                        <span className="text-xs mt-1 font-semibold" style={{ color: '#e8eaf6' }}>
                          {pokemon.name}
                        </span>
                      </button>
                    ))}
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest mb-2" style={{ color: '#4fc3f7', letterSpacing: '0.15em' }}>
                Descripción / Detalle Importante
              </label>
              <textarea
                value={formData.description}
                onChange={e => setFormData(prev => ({ ...prev, description: e.target.value }))}
                placeholder="Historia, logros, o detalles importantes del entrenador..."
                rows={4}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {formData.badges.map((badge, index) => (
              <button
                ...
                onMouseLeave={e => {
                  const el = e.currentTarget as HTMLElement
                  el.style.transform = badge.obtained ? 'scale(1.05)' : 'scale(1)'
                }}
              >
                <img
  src={badge.image}
...
/>
          ...
          <p className="text-xs mt-4 text-center" style={{ color: '#64748b' }}>
            💡 Haz click en cada medalla para marcarla como obtenida
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {isEditing ? 'Actualizar entrenador' : 'Guardar entrenador'}
          </button>
          {isEditing && (
            <>
              <button
                type="button"
                onClick={resetForm}
                className="flex-1 sm:flex-none px-6 py-3 rounded-xl text-sm font-bold tracking-wider uppercase transition-all"
                style={{
      ...
      {/* Existing trainers to edit */}
      {existingTrainers.length > 0 && !isEditing && (
        <div className="mt-8">
          <h3 className="text-lg font-bold mb-4 flex items-center gap-2" style={{ color: '#e8eaf6' }}>
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2">
